...
from tensorflow.keras.applications import (
    EfficientNetB0, ResNet50, MobileNetV2, VGG16
)
...
class WeatherClassificationTrainer:
    ...
    def create_transfer_learning_model(self, base_model_name: str, model_config: Dict) -> Model:
    ...
    def evaluate_model(self, model: Model, splits: Dict) -> Dict:
        ...
        class_names = [self.label_encoder.inverse_transform([i])[0] for i in range(self.num_classes)]
    ...
    def save_model_artifacts(self, model: Model, history: Dict, 
                           evaluation_results: Dict, model_config: Dict) -> str:
        """Save model and training artifacts"""
        logger.info("Saving model artifacts...")
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_name = f"weather_classifier_{model_config.get('model_type', 'cnn')}_{timestamp}"
    ...
    def register_model_mlflow(self, model: Model, model_name: str, 
                            evaluation_results: Dict, model_config: Dict) -> None:
    ...
    def run_training_pipeline(self, model_configs: List[Dict]) -> Dict:
        ...
        for i, model_config in enumerate(model_configs):
            ...
            try:
                ...
                generators = self.create_data_generators(splits, model_config.get('batch_size', 32))
                ...
                if model_type == 'cnn':
                    ...
                    model = self.create_transfer_learning_model(model_type, model_config)
                ...
                # Save artifacts
                model_name = self.save_model_artifacts(model, history, evaluation_results, model_config)
                
                # Register in MLflow
                self.register_model_mlflow(model, model_name, evaluation_results, model_config)
                ...
                logger.info(f"Model {model_name} completed with accuracy: {evaluation_results['accuracy']:.4f}")
        ...
        if best_model:
            best_model_summary = {
                ...
                'all_results': {name: {'accuracy': res['evaluation']['accuracy'], 
            ...
            logger.success(f"Training pipeline completed! Best model: {best_model['model_name']} (Accuracy: {best_accuracy:.4f})")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Model Training for Weather Classification")
    parser.add_argument("--processed_data_path", type=str, default="../artifacts/processed_data",
                       help="Path to processed data")
    parser.add_argument("--models_path", type=str, default="../models",
                       help="Path to save trained models")
    parser.add_argument("--artifacts_path", type=str, default="../artifacts",
                       help="Path to save training artifacts")
    parser.add_argument("--experiment_name", type=str, default="weather_classification",
