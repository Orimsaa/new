...
class DataValidator:
    ...
    def validate_directory_structure(self) -> Dict:
        ...
        logger.info(f"Found {len(validation_results['expected_classes_found'])} expected classes")
    ...
    def validate_images(self) -> Dict:
        ...
        for class_name in self.expected_classes:
            ...
            for image_file in class_path.iterdir():
                if image_file.suffix.lower() not in self.supported_formats:
                    validation_results['unsupported_format_images'].append(str(image_file))
                    continue
                ...
                try:
                    ...
                    validation_results['image_statistics']['height_stats'].append(height)
                    validation_results['image_statistics']['channel_stats'].append(channels)
        ...
        if validation_results['image_statistics']['width_stats']:
            ...
            validation_results['image_statistics'] = {
                ...
                'channels': {
                    'unique': list(set(validation_results['image_statistics']['channel_stats']))
                }
            }
    ...
    def check_class_balance(self, class_counts: Dict) -> Dict:
        ...
        recommendations = []
        if not balanced:
            recommendations.append(f"Dataset is imbalanced (ratio: {imbalance_ratio:.2f}:1)")
    ...
    def generate_validation_report(self, results: Dict) -> str:
        ...
        summary.append(f"Classes Found: {len(results['structure']['expected_classes_found'])}")
        summary.append(f"Dataset Balanced: {results['balance']['balanced']}")
        
        if results['images']['corrupted_images']:
            summary.append(f"⚠️  Corrupted Images: {len(results['images']['corrupted_images'])}")
        
        if results['images']['invalid_size_images']:
            summary.append(f"⚠️  Invalid Size Images: {len(results['images']['invalid_size_images'])}")
    ...
    def run_validation(self) -> Dict:
        ...
        with mlflow.start_run(run_name="data_validation"):
            ...
            # Check class balance
            balance_results = self.check_class_balance(structure_results['class_counts'])
            ...
            complete_results = {
                ...
                'validation_passed': (
                    structure_results['directory_exists'] and
                    len(structure_results['expected_classes_found']) >= 3 and
                    image_results['valid_images'] > 0 and
                    len(image_results['corrupted_images']) == 0
                )
            }
            ...
            mlflow.log_metric("corrupted_images", len(image_results['corrupted_images']))
            mlflow.log_metric("classes_found", len(structure_results['expected_classes_found']))
            mlflow.log_metric("imbalance_ratio", balance_results.get('imbalance_ratio', 0))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Data Validation for Weather Classification")
    ...
    with mlflow.start_run(run_name="weather_data_validation"):
        ...
        mlflow.log_metric("class_imbalance_ratio", results.get('class_imbalance_ratio', 0))
        
        # Log validation status
        validation_status = "Success" if results.get('validation_passed', False) else "Failed"
